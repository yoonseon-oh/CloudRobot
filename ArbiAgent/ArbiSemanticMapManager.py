...
class MapManagerAgent(ArbiAgent):
    ...
    def on_start(self):
        ...
        self.MultiRobotPose_notify(self.NC_name)
    ...
    def MultiRobotPose_notify(self, consumer):
        ...
        RobotPose_gl = " (RobotPose {robot_id} {vertex}"
        vertex_gl = "(vertex_id {v_id1} {v_id2})"
        for id in self.ltm.AMR_IDs:
            if "LIFT" in id:
                temp_vertex = self.ltm.MM.AMR_LIFT[id]["vertex"]
            elif "TOW" in id:
                temp_vertex = self.ltm.MM.AMR_TOW[id]["vertex"]
            
            MultiRobotoPose_gl += RobotPose_gl.format(id, vertex_gl.format(temp_vertex[0], temp_vertex[1]))
        
        self.notify(consumer, MultiRobotoPose_gl)

        threading.Timer(self.threading_timer, self.MultiRobotPose_notify, [consumer]).start()
